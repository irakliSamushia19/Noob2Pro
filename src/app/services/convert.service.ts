...
export class ConvertService {
  ...
  private binToDecSigned(binary: string): number {
    return binary.split('').reduce((acc, bit, idx) => acc + parseInt(bit) * 2 ** (binary.length - 1 - idx), 0);
  }
    ...
    if (typeof input === 'string') {
      ...
      switch (from) {
        ...
        case 'oc':
          ...
          else result = -this.binToDecSigned(bin.slice(1).split('').map(b => b === '0' ? '1' : '0').join(''));
          break;
        case 'tc':
          if (bin[0] === '0') result = this.binToDecSigned(bin);
          else {
      ...
      switch (to) {
        case 'sm':
          ...
          break;
        case 'oc':
          const maxOC = 2 ** (bitLimit - 1) - 1;
          if (num < 0) {
            const inverted = this.padBinary((-num).toString(2), bitLimit - 1).split('').map(b => b === '0' ? '1' : '0').join('');
            output = '1' + inverted;
          } else {
          ...
          break;
        case 'tc':
          const maxTC = 2 ** (bitLimit - 1) - 1;
          if (num < 0) {
            const absVal = Math.abs(num + 1);
            const inverted = this.padBinary(absVal.toString(2), bitLimit).split('').map(b => b === '0' ? '1' : '0').join('');
            output = inverted;
          } else {
            output = this.padBinary(num.toString(2), bitLimit);
          }
